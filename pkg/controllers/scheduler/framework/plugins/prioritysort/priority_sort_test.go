...
func TestPrioritySortSelectClusters(t *testing.T) {
	tests := []struct {
		name             string
	...
	}{
		{
			...
			clusterScoreList: framework.ClusterScoreList{
				...
				{
					Cluster: makeCluster("bar"),
					Score:   2,
				},
			},
			expectedCluster: []string{"bar", "foo"},
			expectedResult:  framework.NewResult(framework.Success),
		},
		{
			name: "2 clusters, 2 have priority",
			su: &framework.SchedulingUnit{
				SchedulingMode: fedcorev1a1.SchedulingModeDuplicate,
			...
			expectedCluster: []string{"foo", "bar"},
			expectedResult:  framework.NewResult(framework.Success),
		},
		{
			name: "2 clusters, 1 has priority",
			su: &framework.SchedulingUnit{
				SchedulingMode: fedcorev1a1.SchedulingModeDuplicate,
				Priorities: map[string]int64{
					"bar": 1,
		...
		{
			name: "2 clusters, 1 has priority, not in clusterScoreList",
			su: &framework.SchedulingUnit{
				SchedulingMode: fedcorev1a1.SchedulingModeDuplicate,
				Priorities: map[string]int64{
					"fun": 1,
		...
		{
			name: "3 clusters, 1 has priority",
			su: &framework.SchedulingUnit{
				SchedulingMode: fedcorev1a1.SchedulingModeDuplicate,
				Priorities: map[string]int64{
					"foo": 1,
			...
			expectedCluster: []string{"foo", "bar", "abc"},
			expectedResult:  framework.NewResult(framework.Success),
		},
	}

	p, _ := NewPrioritySort(nil)
