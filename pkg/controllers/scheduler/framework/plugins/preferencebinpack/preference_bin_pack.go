...
func (pl *PreferenceBinPack) ReplicaScheduling(
	...
	estimatedCapacity, oldEstimatedCapacity := map[string]int64{}, map[string]int64{}
	...
	if autoMigration := su.AutoMigration; autoMigration != nil {
		...
		if info := autoMigration.Info; info != nil {
			...
			for cluster, oldEc := range info.OldEstimatedCapacity {
				if metav1.Now().Time.Before(oldEc.ValidUntil.Time) {
					oldEstimatedCapacity[cluster] = oldEc.EstimatedCapacity
				}
			}
		}
	}
