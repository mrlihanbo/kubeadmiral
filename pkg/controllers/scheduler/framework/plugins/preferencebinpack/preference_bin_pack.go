...
func (pl *PreferenceBinPack) ReplicaScheduling(
	...
	klog.V(4).Infof(
		"[scheduling] for %q clusterPreferences: %s, estimatedCapacity: %v, scheduledPods: %v, currentReplicas: %v, result: %v, overflow: %v",
		su.Key(), spew.Sprint(clusterPreferences), estimatedCapacity, scheduledPods, currentReplicas, scheduleResult, overflow,
	)
