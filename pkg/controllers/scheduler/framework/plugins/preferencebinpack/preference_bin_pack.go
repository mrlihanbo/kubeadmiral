...
func (pl *PreferenceBinPack) ReplicaScheduling(
	...
	var scheduledPods map[string]int64
	if autoMigration := su.AutoMigration; autoMigration != nil {
		if info := autoMigration.Info; info != nil {
			scheduledPods = su.AutoMigration.Info.ScheduledPods
		}
	}
	...
	availableClusters := []string{"kubeadmiral-member-1", "kubeadmiral-member-2", "kubeadmiral-member-3"}
	...
	klog.V(4).Infof(
		"[scheduling] for %q clusterPreferences: %s, estimatedCapacity: %v, currentReplicas: %v, result: %v, overflow: %v",
		su.Key(), spew.Sprint(clusterPreferences), estimatedCapacity, currentReplicas, scheduleResult, overflow,
	)
